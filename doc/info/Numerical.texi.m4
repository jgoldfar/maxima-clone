...
@c -----------------------------------------------------------------------------
@node Introduction to fast Fourier transform, Functions and Variables for fast Fourier transform, , Numerical
...
lengths, consider the @code{fftpack5} package that supports sequences
...
@c -----------------------------------------------------------------------------
@node Functions and Variables for fast Fourier transform, Functions and Variables for FFTPACK5, Introduction to fast Fourier transform, Numerical
@section Functions and Variables for fft
@c -----------------------------------------------------------------------------
...
See also @mref{inverse_fft} (inverse transform),
@mrefcomma{recttopolar} and @mrefdot{polartorect}. See @mref{real_fft}
for FFTs of a real-valued input, and @mref{bf_fft} and
@mref{bf_real_fft} for operations on bigfloat values.  Finally, for
transforms of any size (but limited to float values), see
@mref{fftpack5_fft} and @mref{fftpack5_real_fft}.
...
of @code{real_fft} are purely real.  For larger sequences, @code{real_fft}
...
Computes the inverse Fourier transform of @var{y}, which must have a
...
@c -----------------------------------------------------------------------------
@node Functions and Variables for FFTPACK5, Functions for numerical solution of equations, Functions and Variables for fast Fourier transform, Numerical
@section Functions and Variables for FFTPACK5
@c -----------------------------------------------------------------------------

@code{FFTPACK5} provides several routines to compute Fourier
transforms for both real and complex sequences and their inverses.
The forward transform is defined the same as for @code{fft}.  The
...
efficient when the length has the form @math{2^r*3^s*5^t}.

@code{load("fftpack5")} loads this function.

@anchor{fftpack5_fft}
@deffn {Function} fftpack5_fft (@var{x})

Like @code{fft} (@mref{fft}), this computes the fast Fourier transform
of a complex sequence.  However, the length of @var{x} is not limited
to a power of 2.  
...
@opencatbox{Categories:}
@category{Package fftpack5}
@closecatbox
@end deffn

@anchor{fftpack5_inverse_fft}
@deffn {Function} fftpack5_inverse_fft (@var{y})

Computes the inverse complex Fourier transform, like
@code{inverse_fft}, but is not constrained to be a power of two.
...
@anchor{fftpack5_real_fft}
@deffn {Function} fftpack5_real_fft (@var{x})

Computes the fast Fourier transform of a real-valued sequence @var{x},
just like @code{real_fft}, except the length is not constrained to be
a power of two.
...
for @code{fftpack5_inverse_real_fft}.
...
@anchor{fftpack5_inverse_real_fft}
@deffn {Function} fftpack5_inverse_real_fft (@var{y}, @var{n})

Computes the inverse Fourier transform of @var{y}, which must have a
length of @code{floor(n/2) + 1}.  The length of sequence produced by the
inverse transform must be specified by @var{n}.  This is required
because the length of @var{y} does not uniquely determine @var{n}.
The last element of @var{y} is always real if @var{n} is even, but it
can be complex when @var{n} is odd.
...
@c -----------------------------------------------------------------------------
@node Functions for numerical solution of equations, Introduction to numerical solution of differential equations, Functions and Variables for FFTPACK5, Numerical
